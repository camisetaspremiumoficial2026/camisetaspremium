...
async function cargarProductos() {
    ...
    try {
        const res = await fetch(`${API_URL}/api/productos?categoria=otros`);
        const productos = await res.json();

        if (productos.length === 0) {
            grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;color:#555;">No hay productos disponibles.</div>`;
            return;
        }
        ...
        grid.innerHTML = productos.map(p => `
            <div class="card">
                ...
                <a href="https://wa.me/5493413005198?text=Hola!%20Quiero%20consultar%20por%20${encodeURIComponent(p.nombre)}%20(${p.codigo})" class="btn">Consultar por WhatsApp</a>
            </div>
        `).join('');
